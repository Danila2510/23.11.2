...
DWORD WINAPI WriteToFiles(LPVOID lp)
{
	...
	if (!out.is_open())
	{
		MessageBox(0, TEXT("Error"), TEXT("Critical section"), MB_OK);
		return 1;
	}
	for (int i = 1; i <= 5; i++)
	{
	...
	MessageBox(0, TEXT("Potok information v file"), TEXT("Critical section"), MB_OK);
	return 0;
}
...
DWORD WINAPI ReadToFiles(LPVOID lp)
{
	...
	in.close();
	LeaveCriticalSection(&cs);
	MessageBox(0, TEXT("Potok read information file "), TEXT("Critical section"), MB_OK);
	return 0;
}
